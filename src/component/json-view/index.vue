<!--
 * @FilePath: \maggot-design\src\component\json-view\index.vue
 * @Author: maggot-code
 * @Date: 2022-10-21 16:04:06
 * @LastEditors: maggot-code
 * @LastEditTime: 2022-10-21 16:05:22
 * @Description: 
-->
<template>
            ...
            <div class="content-wrap">
                ...
                <div v-if="!innerclosed && length" class="json-body">
                    <template v-for="(item, index) in items">
                        <json-view v-if="item.isJSON" :closed="isClose()" :key="index" :data="item.value"
                        ...
                        <p class="json-item" v-else :key="index">
                            ...
                            <span :class="['json-value',getDataType(item.value)]">
                                {{`${getDataType(item.value)==='string'?'"':''}${formatValue(item.value)}${getDataType(item.value)==='string'?'"':''}${index
                                ===items.length - 1 ? '' : ','}`}}
                            </span>
                        </p>
                    </template>
